...
pub unsafe fn convert(
    ...
    // This is the core of the change. We check the source process ID of the event.
    // Events generated by `rdev`'s `simulate` function will have the same PID
    // as the current running process. Hardware-generated events will have a PID of 0.
    // By checking if the event's PID matches our own, we can effectively
    // ignore simulated events and prevent an infinite loop.
    let source_pid = cg_event.get_integer_value_field(EventField::EVENT_SOURCE_UNIX_PROCESS_ID);
    if source_pid as u32 == std::process::id() {
        return None;
    }
    // ############ END OF MODIFICATION ############
